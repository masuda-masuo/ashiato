"""The ``ashiato`` command line: build, sql, info."""
...
from ashiato.build import DEFAULT_SOURCE, build, connect, database_info, default_db_path
...
def _run_sql(args: argparse.Namespace, out: Any, err: Any) -> int:
    ...
    if args.format == "json":
        _print_json(columns, rows, out)
    elif args.format == "csv":
        _print_csv(columns, rows, out)
    else:
