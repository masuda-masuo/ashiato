...
def test_format_version_is_unchanged():
    ...
    # rebuild is required.  Bumped to 14 by issue #85: Cursor
    # agent-transcript files now populate sessions / events / tool_calls
    # (one session row per file, one event row per text chunk, one
    ...
    # rebuild is required.
    # Bumped to 16 by issue #87: with `--cursor-chats-source` given, Cursor
    # tool calls now carry outcome / is_error / result_text / result_truncated
