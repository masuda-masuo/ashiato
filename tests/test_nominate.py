...
class TestFormatVersion:
    def test_format_version_is_unchanged(self) -> None:
        ...
        # Bumped to 10 by issue #75: Codex tool_calls rows now carry NULL
        ...
        # is required.  Bumped to 12 by issue #83: opencode events.ndjson
        # files now populate sessions / events / tool_calls (one session row
        # per distinct session id, one event row per text chunk, one
        # tool_calls row per completed tool part) -- the same opencode source
        # bytes yield rows where version 11 derived none, so a rebuild is
        # required.  Bumped to 13 by issue #83: opencode tool parts in the
        # `error` terminal state now produce tool_calls rows with is_error
        # True / outcome 'error' (the failure message from the `error` key
        # lands in result_text), duration_ms is populated from the part's
        ...
        # Bumped to 15 by issue #87: with `--cursor-chats-source` given,
        # Cursor sessions now carry cwd (sessions/events/tool_calls) and the
        # session times (started_at/ended_at) read from the undocumented
        ...
        # carrying the real role, the system prompt and the reasoning -- the
        # same Cursor source bytes yield rows where version 16 kept the
        # role-less transcript text chunks, so a rebuild is required.
        # Bumped to 18 by issues #92/#93: a Cursor store shell result whose
        # first line is a nonzero `Exit code` now classifies as an error, and
        # `CallDynamicTool` blocks are now stored as MCP calls with
